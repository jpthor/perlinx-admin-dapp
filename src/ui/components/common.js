...
import { getTokenSymbol } from '../../client/web3_old'
    ...
    <div style={styles}>
      <Row>
        <Col xs={24}>
          ...
          ></Input>
          <Sublabel>Balance:
            {convertFromWei(props.paneData?.balance)} ({props.paneData?.inputSymbol})</Sublabel>
        </Col>
      </Row>
...
export const TokenDropDown = (props) => {
  ...
  const [symbol, setSymbol] = useState("PERL")
  const [arraySymbols, setArraySymbols] = useState(["PERL"])

  useEffect(() => {
    if (props.tokenList) {
      setSymbol(getTokenSymbol(props.default))
      const symbols = props.tokenList.map((item) => getTokenSymbol(item))
      setArraySymbols(symbols)
    }
    // console.log(props.tokenList, props.default)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.tokenList])

  const handleMenuClick = (e) => {
    setSymbol(getTokenSymbol(props.tokenList[e.key]))
      ...
      <Col xs={24} sm={24} xl={24} style={poolStyles}>
        <Row>
          <Col xs={24}>
            ...
            <Center><Label margin={"0px 0px 0px 0px"}>($2,000.45)</Label></Center>
            <Center><Sublabel margin={"0px 0px 5px 0px"}>DEPTH</Sublabel></Center>
